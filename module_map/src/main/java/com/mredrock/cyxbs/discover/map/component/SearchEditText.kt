...
class SearchEditText : androidx.appcompat.widget.AppCompatEditText {
    ...
    override fun draw(canvas: Canvas?) {
        ...
        //清空按钮
        canvas?.drawBitmap(clearBitmap, offsetXClear + width - clearBitmap.width - 10f, (height - clearBitmap.height) / 2f, Paint())
    ...
    override fun onTouchEvent(event: MotionEvent?): Boolean {
        ...
        if (event?.action == MotionEvent.ACTION_DOWN
                && x >= width - clearBitmap.width - 10f
                && x <= width - clearBitmap.width - 10f + clearBitmap.width
